...
void downHeap(Node *n) {
  ...
  if(n->left == nullptr && n->right == nullptr){
    ...
    smallestChild = n->left;
  }
  else if(n->left == nullptr){
    smallestChild = n->right;
  }
  else {
    smallestChild= n->left->value < n->right->value ? n->left : n->right;
  }
  
  if(n->value > smallestChild->value){  
    int temp = n->value;
    n->value = smallestChild->value;
