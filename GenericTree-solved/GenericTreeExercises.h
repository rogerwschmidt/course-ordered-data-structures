...
static void treeFactory(GenericTree<int>& tree) {
  GenericTree<int>::TreeNode* r = tree.getRootPtr();
  r->data = 4;

  GenericTree<int>::TreeNode* eight= r->addChild(8);
  r->addChild(15);

  GenericTree<int>::TreeNode* sixteen = eight->addChild(16);
  GenericTree<int>::TreeNode* twentyThree = eight->addChild(23);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  treeQueue.push(rootNodePtr);

  while(treeQueue.size() > 0){
    auto top = treeQueue.front();
    treeQueue.pop();
    
    results.push_back(top->data);

    for(unsigned long i = 0; i < top->childrenPtrs.size(); i++){
      treeQueue.push(top->childrenPtrs[i]);
    }
  }
